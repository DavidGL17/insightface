...
class RandomFlip(object):
    ...
    def __call__(self, results):
        ...
        if results["flip"]:
            ...
            for key in results.get("keypoints_fields", []):
                results[key] = self.keypoints_flip(results[key], results["img_shape"], results["flip_direction"])
            # flip masks
            for key in results.get("mask_fields", []):
    ...
    def __call__(self, results):
        ...
        while True:
            ...
            for i in range(250):
                ...
                patch = np.array((int(left), int(top), int(left + cw), int(top + ch)), dtype=int)
...
class PhotoMetricDistortion(object):
    ...
    def __init__(
        self, brightness_delta=32, contrast_range=(0.5, 1.5), saturation_range=(0.5, 1.5), hue_delta=18, gray_prob=0.0
    ):
